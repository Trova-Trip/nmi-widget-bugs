...
// Public NMI tokenization key, supplied via env (see .env.example).
const TOKENIZATION_KEY = import.meta.env.VITE_NMI_TOKENIZATION_KEY ?? '';

// The vendor hides the spinner + fires onFieldsAvailable only after all three
        ...
        <div>
            ...
            >
                ...
                <div style={{ display: 'flex', gap: 12, fontSize: 13 }}>
                    {REQUIRED_FIELDS.map((f) => (
                        <span key={f} style={{ color: seenFields.has(f) ? '#0a7d2c' : '#999' }}>
                            {seenFields.has(f) ? '✓' : '○'} {f}
                        </span>
                    ))}
                </div>
                ...
                <details style={{ marginTop: 8, fontSize: 12 }}>
                    <summary style={{ cursor: 'pointer' }}>timeline ({timeline.length})</summary>
            ...
            <NmiPayments
                ...
                onChange={(data: PaymentChangeEvent) => {
                    // Capture the token exactly like microapps' useNmiForm.
                    setPaymentToken(data.complete ? data.token : undefined);
                    console.log('[repro] onChange', data);
                }}
            />

            {/* Single Pay button drives the 3DS flow — mirroring microapps,
                which uses its own submit button, not the vendor pay button. */}
            <button
            ...
            {threeDSStatus !== 'idle' && (
                ...
                >
                    <div style={{ fontWeight: 700, color: THREE_DS_COLORS[threeDSStatus] }}>
